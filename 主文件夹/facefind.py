...
def XunLian():
    ...
    for Name in NameList:
        ...
        for image in imagesDir:
            ...
            imageFace = FacesFind(imageFace)
            imageFace = cv.cvtColor(imageFace,cv.COLOR_BGR2GRAY)
            imageFace = cv.resize(imageFace,(102,102))
...
def DisPaly(img,faceimg):
    if faceimg==0:
        cv.imshow('dispaly',img)
    else:
        faceimg = cv.resize(faceimg,(100,100))
        img[50:150,50:150]=faceimg
        cv.imshow('dispaly',img)
